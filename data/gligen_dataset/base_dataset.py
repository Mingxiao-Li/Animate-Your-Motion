...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, random_crop, random_flip, image_size, crop_ratio):
        super().__init__() 
    ...
    def random_crop_and_resize(self, pil_image,image_size):
        WW, HH = pil_image.size 
        s = min(*pil_image.size)
        crop_size = random.random() * ((1-self.crop_ratio)*s) + self.crop_ratio*s

        scale = image_size / crop_size

        # pil image (y,x ) np.array (x,y)
        crop_x = random.random()*(WW-crop_size)
        crop_y = random.random()*(HH-crop_size)
        img = pil_image.crop((crop_x,crop_y, crop_x+crop_size, crop_y +crop_size))
        img = img.resize((image_size, image_size), resample=Image.BOX)
        
    
        info = {"performed_scale": scale, 'crop_y': crop_y*scale, 'crop_x': crop_x*scale,'Ww':WW, 'HH': HH}
        arr = np.array(img)
        return arr, info
    ...
    def transform_image(self, pil_image, ):
        ...
        if self.random_crop:
            arr, info = self.random_crop_and_resize(pil_image,self.image_size)
            # assert False
