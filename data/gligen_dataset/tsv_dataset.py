...
class TSVDataset(BaseDataset):
    def __init__(self, 
        ...
        super().__init__(random_crop, random_flip, image_size, crop_ratio)
    ...
    def __getitem__(self, index):
        ...
        if self.cat_input:
            ...
            cur_aug = random.choice(augmentation_methods)
            cond_img = cur_aug(image)
            cond_img_tensor, _ = self.transform_image(cond_img)
            out["cond_pixel"] = cond_img_tensor
        else:
            out["cond_pixel"] = None
     
        image_tensor, trans_info = self.transform_image(image)
        out["pixel_values"] = image_tensor
        ...
        if random.uniform(0, 1) < self.prob_use_caption:
            if is_det:
                caption = make_a_sentence(category_names)
            else:
                caption = raw_item["caption"]
        else:
            caption = ""
        out["prompt_text"] = caption
        tokens = self.tokenizer(
            caption, max_length=self.text_max_length, padding="max_length", truncation=True, return_tensors="pt"
        )
        out["prompt_ids"] = tokens.input_ids[0]
        out["prompt_padding_mask"] = tokens.attention_mask[0]
        return out
